...
class ApiService {
  ...
  bool _statisticsServedStale = false;

  static const String _statisticsPayloadKey = 'statistics.snapshot.payload';
  static const String _statisticsEtagKey = 'statistics.snapshot.etag';
  ...
  Future<SearchStatistics?> fetchSearchStatistics(Filters filters) {
    ...
    request.then(
      (value) {
        final retry = value == null || _statisticsServedStale;
        if (retry && identical(_statisticsSnapshotRequest, request)) {
  ...
  /// The one network call behind [fetchSearchStatistics]. Exposed so tests can
  /// answer it without a live backend.
  @visibleForTesting
  ...
  /// with its ETag and ask conditionally: an unchanged snapshot comes back as an
  /// empty 304 rather than the whole aggregate, and a cold app start renders
  /// from the stored copy instead of waiting on the network. The copy is also
  /// what we fall back to when the request fails outright, so the statistics
  /// sheet still has something to show offline.
  Future<SearchStatistics?> _fetchSearchStatisticsSnapshot() async {
    SharedPreferences? prefs;
    try {
      prefs = await SharedPreferences.getInstance();
    } catch (_) {
      // Storage unavailable: fall through to an unconditional request.
    }
    final cachedBody = prefs?.getString(_statisticsPayloadKey);
    final cachedEtag = prefs?.getString(_statisticsEtagKey);

    http.Response res;
    try {
      res = await sendStatisticsRequest(
      ...
      _statisticsServedStale = true;
      return _decodeStatistics(cachedBody);
    }

    // 304 means our stored copy is still the current generation; any other
    // non-200 is an outage we would rather ride out on the stored copy.
    if (res.statusCode != 200) {
      _statisticsServedStale = res.statusCode != 304;
      return _decodeStatistics(cachedBody);
    }
    ...
    if (decoded is! Map || decoded['statistics'] is! Map) {
      _statisticsServedStale = true;
      return _decodeStatistics(cachedBody);
    }
    final stats = decoded['statistics'] as Map;
    _statisticsServedStale = false;

    final payload = Map<String, dynamic>.from(stats);
    final etag = res.headers['etag'];
    if (prefs != null) {
      await prefs.setString(_statisticsPayloadKey, jsonEncode(payload));
      if (etag != null && etag.isNotEmpty) {
        await prefs.setString(_statisticsEtagKey, etag);
      } else {
        await prefs.remove(_statisticsEtagKey);
      }
    }
    return SearchStatistics.fromJson(payload);
  }

  SearchStatistics? _decodeStatistics(String? stored) {
    if (stored == null || stored.isEmpty) return null;
    try {
      final decoded = jsonDecode(stored);
      if (decoded is! Map) return null;
      return SearchStatistics.fromJson(Map<String, dynamic>.from(decoded));
    } catch (_) {
      return null;
    }
  }
