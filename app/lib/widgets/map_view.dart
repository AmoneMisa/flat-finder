...
class _MapViewState extends State<MapView> with SingleTickerProviderStateMixin {
  ...
  PriceTone _priceToneForGroup(List<MapListingPoint> listings) {
            ...
            return Column(
              ...
              children: [
                SwitchListTile.adaptive(
                  ...
                  subtitle: available
                      ? null
                      : Text(_mapCopy(context, 'нет данных', 'no data')),
          ...
          if (group == _MapLayerGroup.territories) {
            children.addAll([
              toggle(
                  ...
                  onChanged: (v) => update(() => _showCity = v)),
              toggle(
                  label: s.t('districts'),
            ...
            children.addAll([
              toggle(
                  ...
                  value: _showMetro,
                  available: _zones.metroStations.isNotEmpty,
                  onChanged: (v) => update(() => _showMetro = v),
                  radius: _shapeRadiusM(context.read<AppState>().filters)
                      .toDouble(),
                  onRadius: (v) {
                    final state = context.read<AppState>();
                    update(() {});
                    ...
                    unawaited(
                        state.search().then((_) => state.loadMapListings()));
                  }),
              toggle(
                  label: _mapCopy(context, 'Автобусы', 'Bus'),
                  icon: Icons.directions_bus_outlined,
                  value: _showBus,
                  available: _zones.transport('bus').isNotEmpty,
                  onChanged: (v) => update(() => _showBus = v),
                  radius: _busRadiusM,
                  onRadius: (v) => update(() => _busRadiusM = v)),
              toggle(
                  label: _mapCopy(context, 'Трамвай', 'Tram'),
                  icon: Icons.tram_outlined,
                  value: _showTram,
                  available: _zones.transport('tram').isNotEmpty,
                  onChanged: (v) => update(() => _showTram = v),
                  radius: _tramRadiusM,
                  onRadius: (v) => update(() => _tramRadiusM = v)),
              toggle(
                  label: _mapCopy(context, 'Троллейбус', 'Trolleybus'),
                  icon: Icons.directions_bus_outlined,
                  value: _showTrolleybus,
                  available: _zones.transport('trolleybus').isNotEmpty,
                  onChanged: (v) => update(() => _showTrolleybus = v),
                  radius: _trolleybusRadiusM,
                  onRadius: (v) => update(() => _trolleybusRadiusM = v)),
              toggle(
                  label: _mapCopy(context, 'Маршрутки', 'Minibus'),
                  icon: Icons.airport_shuttle_outlined,
                  value: _showMinibus,
                  available: _zones.transport('minibus').isNotEmpty,
            ...
            children.addAll([
              toggle(
                  label: _mapCopy(context, 'ЖК', 'Residential complexes'),
                  icon: Icons.apartment_outlined,
                  value: _showResidentialComplexes,
                  available: _zones.residentialComplexes.isNotEmpty,
                  onChanged: (v) =>
                      update(() => _showResidentialComplexes = v)),
              toggle(
                  label: _mapCopy(context, 'Школы', 'Schools'),
              ...
              child: ListView(
                ...
                children: [
                  Padding(
                    ...
                    child: Text(
                      switch (group) {
                        _MapLayerGroup.territories =>
                          _mapCopy(context, 'Территории', 'Territories'),
                        _MapLayerGroup.transport =>
                          _mapCopy(context, 'Транспорт', 'Transport'),
                        _MapLayerGroup.poi => _mapCopy(
                            context, 'Объекты и сервисы', 'Places & services'),
                      },
            ...
            child: FlutterMap(
              ...
              children: [
                ...
                if (_showMetro && _selectedMetroStations(filters).isNotEmpty)
                  PolygonLayer(
                    polygons: [
                      for (final station in _selectedMetroStations(filters))
                        Polygon(
                          points: sectorPolygon(
                            LatLng(station.lat, station.lng),
                            _shapeRadiusM(filters).toDouble(),
                            from: _shapeBearingFrom(filters)?.toDouble(),
                ...
                if (_showParks && _zones.parks.isNotEmpty)
                  PolygonLayer(polygons: [
                    for (final poi in _zones.parks)
                      _proximityRing(
                          poi, _parkRadiusM, _parseHexColor(poi.colorHex)),
                    for (final poi in _zones.parks)
                ...
                if (_showShoppingMalls && _zones.shoppingMalls.isNotEmpty)
                  PolygonLayer(polygons: [
                    for (final poi in _zones.shoppingMalls)
                      _proximityRing(
                          poi, _mallRadiusM, _parseHexColor(poi.colorHex)),
                    for (final poi in _zones.shoppingMalls)
                      for (final ring in poi.boundaryRings)
                        _zonePolygon(poi, ring, fillAlpha: 0.09),
                  ]),
                if (_showUniversities && _zones.universities.isNotEmpty)
                  PolygonLayer(polygons: [
                    for (final poi in _zones.universities)
                      _proximityRing(poi, _universityRadiusM,
                          _parseHexColor(poi.colorHex)),
                    for (final poi in _zones.universities)
                      for (final ring in poi.boundaryRings)
                        _zonePolygon(poi, ring, fillAlpha: 0.09),
                  ]),
                if (_showSchools && _zones.schools.isNotEmpty)
                  PolygonLayer(polygons: [
                    for (final poi in _zones.schools)
                      _proximityRing(
                          poi, _schoolRadiusM, _parseHexColor(poi.colorHex))
                  ]),
                if (_showAirports && _zones.airports.isNotEmpty)
                  PolygonLayer(polygons: [
                    for (final poi in _zones.airports)
                      _proximityRing(
                          poi, _airportRadiusM, _parseHexColor(poi.colorHex))
                  ]),
                if (_showRailwayStations && _zones.railwayStations.isNotEmpty)
                  PolygonLayer(polygons: [
                    for (final poi in _zones.railwayStations)
                      _proximityRing(
                          poi, _railwayRadiusM, _parseHexColor(poi.colorHex))
                  ]),
                if (_showBusStations && _zones.busStations.isNotEmpty)
                  PolygonLayer(polygons: [
                    for (final poi in _zones.busStations)
                      _proximityRing(
                          poi, _busStationRadiusM, _parseHexColor(poi.colorHex))
                  ]),
                if (_showParkings && _zones.parkings.isNotEmpty)
                  PolygonLayer(polygons: [
                    for (final poi in _zones.parkings)
                      _proximityRing(
                          poi, _parkingRadiusM, _parseHexColor(poi.colorHex))
                  ]),
                              ...
                              child: GestureDetector(
                                ...
                                child: _MetroStationMarker(
                                  ...
                                  lineColors: [
                                    ...
                                    if (station.lineColorHexes.isEmpty &&
                                        station.lineColorHex != null)
                ...
                if (_showSchools && _zones.schools.isNotEmpty)
                  MarkerLayer(
                      markers:
                          _poiMarkers(_zones.schools, Icons.school_outlined)),
                ...
                if (_showAirports && _zones.airports.isNotEmpty)
                  MarkerLayer(
                      markers:
                          _poiMarkers(_zones.airports, Icons.flight_outlined)),
                if (_showRailwayStations && _zones.railwayStations.isNotEmpty)
                  MarkerLayer(
                      markers: _poiMarkers(
                          _zones.railwayStations, Icons.train_outlined)),
                if (_showBusStations && _zones.busStations.isNotEmpty)
                  MarkerLayer(
                      markers: _poiMarkers(
                          _zones.busStations, Icons.departure_board_outlined)),
                if (_showParkings && _zones.parkings.isNotEmpty)
                  MarkerLayer(
                      markers: _poiMarkers(
                          _zones.parkings, Icons.local_parking_outlined)),
                if (_visibleTransportStops.isNotEmpty)
            ...
            child: Card(
              child: Row(
                children: [
                  ...
                  Text(
                      '${(widget.radiusM! / 1000).toStringAsFixed(1)} ${s.kilometresShort}'),
                  const SizedBox(width: 12),
                ],
              ),
            ),
        ...
        Positioned(
          ...
          child: Row(
            ...
            children: [
              ...
              _MapLayerButton(
                tooltip:
                    _mapCopy(context, 'Объекты и сервисы', 'Places & services'),
...
class _MetroStationMarker extends StatelessWidget {
  const _MetroStationMarker(
      {required this.selected, required this.lineColors, this.dimmed = false});
  final bool selected;
  final bool dimmed;
...
class _MetroRingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < colors.length; i++) {
      ...
      canvas.drawArc(
          rect.deflate(1.5), -math.pi / 2 + sweep * i, sweep, false, paint);
    }
  }
