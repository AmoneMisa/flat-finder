...
test('parses bare USD price from Uzbek Telegram rental without dealType metadata', () => {
  assert.deepEqual(parseTelegramPrice(problematicPost, UZ), {
    price: 500,
    currency: 'USD',
  });
});
...
test('bare-price fallback ignores area/block numbers and explicit sale posts', () => {
  assert.deepEqual(parseTelegramPrice('Yunusobod 500 m2 hovli sotiladi', UZ), {
    price: null,
    currency: 'UZS',
  });
  assert.deepEqual(parseTelegramPrice('Chilonzor 500 kvartal 3 xona', UZ), {
    price: null,
    currency: 'UZS',
  });
  assert.deepEqual(parseTelegramPrice('Shayxontohur 500 Samarqand Darvoza hovli sotiladi', UZ), {
    price: null,
    currency: 'UZS',
  });
});
